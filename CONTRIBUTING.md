...
Le projet est publié sous **Apache-2.0**. Toute contribution doit rester cohérente avec cette licence.
...
- [ ] Les badges de licence dans `README.md` (si présents) pointent vers **Apache-2.0**.
- [ ] Les templates GitHub (issues/PR) et la documentation (`docs/`) ne contiennent aucune information contradictoire sur la licence.
- [ ] Les nouveaux fichiers source incluent un en-tête SPDX valide : `SPDX-License-Identifier: Apache-2.0`.
- [ ] `LICENSE`, `NOTICE` et `package.json` restent alignés sur **Apache-2.0**.
